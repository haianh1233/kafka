...
public class SocketServerConfigs {
    ...
    public static final String LISTENERS_DOC = String.format("Listener List - Comma-separated list of URIs we will listen on and the listener names." +
                    ...
                    " Supported protocols: PLAINTEXT, SSL, SASL_PLAINTEXT, SASL_SSL, HTTP, HTTPS." +
                    " HTTP and HTTPS listeners start a Netty-based HTTP server for REST API access (produce, consume, metadata)." +
                    " HTTP/HTTPS listeners cannot be used as the inter-broker listener.%n" +
                    ...
                    " <code>PLAINTEXT://0.0.0.0:9092,HTTP://0.0.0.0:9094</code>%n", LISTENER_SECURITY_PROTOCOL_MAP_CONFIG);
