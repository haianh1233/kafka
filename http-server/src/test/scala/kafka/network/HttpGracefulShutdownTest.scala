...
class HttpGracefulShutdownTest {
  ...
  def testMultipleRequestsDuringDrainAllGet503(): Unit = {
    ...
    // Each drain 503 response includes Connection: close which closes the channel.
    // Verify that multiple separate channels each get 503.
    for (i <- 1 to 3) {
      val handler = new HttpRequestHandler(
        principalBuilder, SecurityProtocol.HTTP, draining, inFlightCount)
      val ch = new EmbeddedChannel(handler)

      val request = buildRequest(HttpMethod.GET, s"/v1/health?req=$i")
      ch.writeInbound(request)

      val response = ch.readOutbound[FullHttpResponse]()
